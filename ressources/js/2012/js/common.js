...
sensorCalibration = { /*
						 * An array containing the name of the data and a, b as
						 * calibratedData = (data * a) + b
						 */
			...
			.done(
					function(text) {
						try {
							...
							alert('Le fichier "'
									+ file
									+ '" contient une erreur de syntaxe qui n\'a pas pu être corrigée.\n\nErreur:\n'
									+ e
...
function errorAccessFile() {
	alert("Erreur d'accès au fichier '" + UBPE.dataFile
			+ "', actualisez la page.");
}
...
function filterData(dataArg) {
	...
	for (i in sensorCalibration) {
		var sensor = data[i];
		if (sensor != null) {
			data[i] = (data[i] * sensorCalibration[i][0])
					+ sensorCalibration[i][1];
		} else
			data[i] = 0;
	}
